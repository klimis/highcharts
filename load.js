(function(win, doc) {
    ...
    $wt.extend({
        ...
        force: function(params) {
            return (!!$wt.url.params.get('wtrender') || params.render || ["laco", "cck", "globan", "announcement", "etrans", "sbkm", "share"].indexOf(params.service) !== -1)
        },
        ...
        runComponent: function(comp, scrpt, params) {
            ...
            if (translations && !scrpt.translation) {
                scrpt.translation = "inProgress";
                return $wt.loadTranslations(translations, (what) => {
                    if (scrpt.translation === "RunOnLoad") {
                        $wt.runComponent(comp, scrpt, params)
                    }
                    scrpt.translation = "Done"
                }
                , comp)
            } else if (scrpt.translation === 'inProgress') {
                scrpt.translation = "RunOnLoad";
                return !1
...
$wt.extend({
    ...
    label: function(set, label, lang, placeholder, range, initialValue) {
    ...
    addTranslation: function(json, force=!1) {
    ...
    getFile: async (params={}) => {
        ...
        const batchGeocodingService = (geojson, key) => {
            if (geojson.features.length > 5000) {
                return params.error("The geocoding batch service is limited to 5000 requests.")
            }
        ...
        if (/(xlsx?|ods)/i.test(params.type)) {
            ...
            if (!params.options.proxy && (isGeojson || params.options.to === "json" || params.options.to === "tables")) {
                let info = {};
...
( (frame) => {
    ...
    $wt.frame = {
        ...
        resize: () => {
            ...
            if (!document.getElementById("wt-frame-fix")) {
        ...
        parent: function(e) {
            ...
            if (data.service === 'cck-check-network') {
                ...
                if ($wt.laco) {
                    ...
                    $wt.load([$wt.root + ((isDyn) ? '' : '/css') + "/webtools.laco.css?t=1743409332", $wt.root + ((isDyn) ? '' : '/js') + "/webtools.laco.js?t=1743409332"], () => {
                        $wt.laco.openModal(data)
                    }
                    )
                }
    ...
    if ($wt.exists("cck")) {
        window.addEventListener("wtCckReady", () => {
            if ($wt.cookie.consent.is.allAccepted()) {
                ...
                window.addEventListener("cck_technical_accepted", () => isDynatrace.disable())
            }
        }
        )
    } else {
...
$wt.print = (container, options={}) => {
    ...
    params.filename = params.filename.replace(/(\s+|-|_|\|)/ig, '-').replace(/-+/ig, '-').toLowerCase();
    return new Promise( (resolve, reject) => {
        const response = (dataUrl) => {
            if (params.filetype === "pdf" && !isURL) {
                ...
                if (!window.wtPrint) {
                    ...
                    window.wtPrint.setAttribute("style", "background-color:#fff;position:absolute;left:-900px;top:-900px;width:0;height:0;")
                }
                window.wtPrint.focus();
                let doc = window.wtPrint.contentWindow.document;
                doc.open();
                doc.write('<img style="max-width: 100%;" class="wt-img" onload="document.execCommand(\'print\', false, null)||window.print();" src="' + dataUrl + '" border="0">');
                doc.close()
            }
            resolve()
        }
        ...
        $wt.include($wt.root + '/libs/dom-to-image-more/dom-to-image-more.min.js', () => {
            if (params.filetype !== "pdf" && params.scale === 1) {
                params.scale = 2
            }
            let engine = (params.filetype === "png") ? domtoimage.toPng : domtoimage.toJpeg;
            engine(container, params).then(response).catch( (e, m) => {
...
(function(win, doc, previous) {
    ...
    aria.setAttributes = (elm, params={}) => {
        Object.keys(params).forEach(prop => {
            elm.setAttribute('aria-' + $wt.cleanHTML(prop), $wt.cleanHTML(params[prop].toString()))
        }
        )
    }
    ;
    ...
    ;
    $wt.aria.autocomplete = (field, options={
        showOnFocus: !1
    }) => {
        let timer;
        ...
        let spinner = '<span class="wt-icon--spinner wt-icon--rotate-animate wt-icon--fluid"></span>';
        let menuItems = [];
        let firstItem = !1;
        let lastItem = !1;
        let hover = !1;
        ...
        const searchAndResult = async () => {
            ...
            if (term.length > 2 || options.showOnFocus) {
                [...document.querySelectorAll('.wt-autocomplete--wrapper')].forEach(elm => elm.close());
                menuItems = [];
...
$wt.on(window, "wtBeforeCollect", () => {
    ...
    const iconsRefresh = group => {
        let icons = document.querySelectorAll(`[class^=${group.match}]`);
        if (!icons.length || inProgress) {
            return
        }
        inProgress = !0;
        const replaceIcons = () => {
            [...icons].forEach(elm => {
                ...
                let xhtml = window.getComputedStyle(document.body).getPropertyValue('--' + fullName);
                if (xhtml) {
                    ...
                    if (group.name === 'flag') {
                        dom.classList.add('wt-icon-flag-low')
                    } else if (group.name === 'icons') {
        ...
        ;
        $wt.include($wt.root + `/css/webtools.icons.${group.file}.css`, replaceIcons)
    }
    ;
    const loop = () => family.forEach(iconsRefresh);
    $wt.domchange(loop);
    $wt.on(window, "wtNext", () => setTimeout(loop, 25));
    loop()
}
);
$wt.on(window, "wtBeforeCollect", () => {
...
$wt.on(window, "wtReady", () => {
    ...
    function isIpAddress(input) {
        ...
        const ipv6Regex = /^(([0-9a-fA-F]{1,4}:){7}([0-9a-fA-F]{1,4}|:)|(([0-9a-fA-F]{1,4}:){1,7}:)|([0-9a-fA-F]{1,4}:){1,6}(:[0-9a-fA-F]{1,4}){1,6})$/;
...
$wt.ready(function() {
    ...
    let theme = $wt.url.params.get("theme", SML.src) || null;
    $wt.theme = $wt.setTheme(theme);
    $wt.spa = !!$wt.url.params.get("spa", SML.src) || !1;
    $wt.infogram = $wt.url.params.get("infogram", SML.src) || !1;
    $wt.root = (url.startsWith('https://europa.eu/webtools')) ? "https://webtools.europa.eu" : url;
    $wt.lang();
